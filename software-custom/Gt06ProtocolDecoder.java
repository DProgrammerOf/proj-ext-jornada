...
//                return Position.ALARM_JAMMING;
    ...
    private Object decodeBasic(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (type == MSG_LOGIN) {
            ...
            DateBuilder dateBuilder = new DateBuilder((TimeZone) deviceSession.getTimeZone())
                    .setDate(buf.readUnsignedByte(), buf.readUnsignedByte(), buf.readUnsignedByte())
                    .setTime(buf.readUnsignedByte(), buf.readUnsignedByte(), buf.readUnsignedByte());
            ...
            if (type == MSG_GPS_LBS_STATUS_1 && variant == Variant.STANDARD) {
                Device device = Context.getDeviceManager().getById(deviceSession.getDeviceId());
                // Alarms igntion to GS900 model
                if (device.getDecode_alarm().equals("GS900")) {

                    // decode gps part
                    decodeGpsVariant(position, deviceSession, buf, device.getDecode_alarm());

                    // decode lbs part
                    decodeLbsVariant(position, buf, device.getDecode_alarm());

                    // decode status part
                    decodeStatusVariant(position, buf, device.getDecode_alarm());
    ...
    private Object decodeExtended(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (type == MSG_STRING_INFO) {
            ...
            if (subType == 0x00) {
                ...
                // 1 byte = header
                // 1 byte = family
                short info_crc = buf.readUnsignedByte();
                ByteBuf info_data = buf.readBytes(4);
                short info_type = buf.readUnsignedByte();
                short info_header = buf.readUnsignedByte();
                short info_family = buf.readUnsignedByte();

                // Informações relacionado ao dispositivo Arduino (1-wire)
                if (
                    info_data.getByte(0) == 0x0 && info_data.getByte(1) == 0x0
                    && info_data.getByte(2) == 0x0 && info_data.getByte(3) == 0x0
                    && info_header == 1 && info_family == 1
                ) {
                    position.set(Position.KEY_ALARM, Event.TYPE_SERIAL);

                    switch (info_type) {
                        case 1:
                            position.set("serial", "InicioJornada");
                            break;
                        case 2:
                            position.set("serial", "FimJornada");
                            break;
                        case 3:
                            position.set("serial", "Almoco");
                            break;
                        default:
                            position.set("serial", info_type);
                            break;
                    }
                }
                // Código para outros dispositivos (original)
                else if (Character.isLetter(info_crc)) {
                    String data = buf.readCharSequence(info_data.readableBytes(), StandardCharsets.US_ASCII).toString();
                    data += (char) info_type;
                    data += (char) info_header;
                    position.set("serial", data);
                }
                // Código para i-buttons (original)
                else {
                    position.set(Position.KEY_DRIVER_UNIQUE_ID, ByteBufUtil.hexDump(info_data));
                }
    ...
    private boolean decodeStatusVariant(Position position, ByteBuf packet, String name) {
        if (name.equals("GS900")) {
            short terminal = packet.readUnsignedByte();
            position.set(Position.KEY_STATUS, terminal);
            position.set(Position.KEY_IGNITION, BitUtil.check(terminal, 1));
            position.set(Position.KEY_CHARGE, BitUtil.check(terminal, 2));
            position.set(Position.KEY_BLOCKED, BitUtil.check(terminal, 7));
            // terminal content

            int voltage_bat = packet.readUnsignedByte();
            double voltage = 0;
            if (((voltage_bat & 0x80) == 0x80)) {
                position.set(Position.KEY_BATTERY_LEVEL, 100);
                position.set(Position.KEY_BATTERY, 4.5);
                voltage = ((((voltage_bat) & 0x0F) | voltage_bat) / 10.0);
            } else {
                int batpct = (voltage_bat*100) / 6;
                position.set(Position.KEY_BATTERY_LEVEL, batpct);
                position.set(Position.KEY_BATTERY, (voltage_bat*4.5) / 6);
            }
            position.set(Position.KEY_VIN, voltage);
            // voltage e level battery

            int signal = packet.readUnsignedByte();
            position.set(Position.KEY_RSSI, signal);
            // gsm signal strength

            short alarm = packet.readUnsignedByte();
            if (alarm == 0xFE) {
                position.set(Position.KEY_ALARM, Event.TYPE_IGNITION_ON);
            }
            else if (alarm == 0xFF) {
                position.set(Position.KEY_ALARM, Event.TYPE_IGNITION_OFF);
            }
            else if (alarm == 0x29) {
                position.set(Position.KEY_ALARM, Position.ALARM_ACCELERATION);
            }
    ...
    private boolean decodeLbsVariant(Position position, ByteBuf packet, String name) {
        if (name.equals("GS900")) {
            short lbs_length = packet.readUnsignedByte();
            if (lbs_length == 8) {
                int mcc = packet.readUnsignedShort();
                int mnc = BitUtil.check(mcc, 15) ? packet.readUnsignedShort() : packet.readUnsignedByte();
                int lac = packet.readUnsignedShort();
                int cellid = packet.readUnsignedMedium();
                position.setNetwork(new Network(CellTower.from(
                    BitUtil.to(mcc, 15), mnc, lac, cellid))
                );
            }
    ...
    private boolean decodeGpsVariant(Position position, DeviceSession deviceSession, ByteBuf packet, String name) {
        if (name.equals("GS900")) {
            ByteBuf datetime_bytes = packet.readBytes(6);
            // get datetime
            short gps_information_bytes = packet.readUnsignedByte();
            // get gps length and quantity satellites
            double latitude_bytes = packet.readUnsignedInt();
            double longitude_bytes = packet.readUnsignedInt();
            // get latitude and longitude
            short speed_bytes = packet.readUnsignedByte();
            // get speed
            int flags = packet.readUnsignedShort();
            // get course (10 bits) and status


            DateBuilder dateBuilder = new DateBuilder(deviceSession.getTimeZone())
                    .setDate(datetime_bytes.readUnsignedByte(), datetime_bytes.readUnsignedByte(), datetime_bytes.readUnsignedByte())
                    .setTime(datetime_bytes.readUnsignedByte(), datetime_bytes.readUnsignedByte(), datetime_bytes.readUnsignedByte());
            // format datetime
            int length = BitUtil.to(gps_information_bytes, 4);
            int satellites = BitUtil.between(gps_information_bytes, 4, 8);
            // gps information
            double speed = UnitsConverter.knotsFromKph(speed_bytes);
            // speed
            double course = BitUtil.to(flags, 10);
            // course
            boolean valid = BitUtil.check(flags, 12);
            // GPS having been positioning or not
            double latitude = latitude_bytes / 60.0 / 30000.0;
            double longitude = longitude_bytes / 60.0 / 30000.0;
            if (!BitUtil.check(flags, 10)) {
                latitude = -latitude;
            }
            if (BitUtil.check(flags, 11)) {
            ...
            boolean ignition = BitUtil.check(flags, 14);
            // ignition
            ...
            position.setValid(true);
            position.set("valid", valid);
            position.set(Position.KEY_SATELLITES, satellites);
            position.set(Position.KEY_IGNITION, ignition);
            position.setTime(dateBuilder.getDate());
            // update position infos

            System.out.println("@@@@ GPS900 finish gps decoded");
            return true;
        }
