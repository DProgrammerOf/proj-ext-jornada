...
uint8_t OneWireHub::attach(OneWireItem &sensor)
{
    ...
    device_list[0] = &sensor;
    device_count = 1;
    buildIDTree();
    return 0;
}

bool OneWireHub::detach()
{
    if (device_count == 0) return false;

    device_list[0] = nullptr;
    device_count = 0;
    buildIDTree();
...
bool OneWireHub::poll(uint8_t *in_action)
{
    _error = Error::NO_ERROR;
    ...
    while (true)
    {
        ...
        // Remove o serial da ação e reseta a variável para entrada de outro
        *in_action = 0xff;
        detach();
